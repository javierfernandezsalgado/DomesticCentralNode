...
typedef struct
{
	const uint8_t char *ssid;
	const uint8_t char *passw;
	uint16_t            tcp_port;
...
/*Rele Operations*/
static bool      isSwitchOnRele(uint16_ nodeIndex,void);
...
void setup() {
	...
	result == 0u  ? :send_tm_initInvalid(result);

	Serial.println("Configure Timers");
	(result == 0u) && 	(configureTimers());
...
static void     configureTimers(void)
{
	gTempTicker.attach_ms_scheduled(500, command_manager);
	gTempTicker.attach_ms_scheduled(5100, HKNodes);
}
...
static void command_manager (void)
{
	...
	/*Verify  connections*/
	WifiClient client = gWifiServer.avalaible();
	uint16_t counter = 0u;
	uint8_t  command [20u];
									...
									case GETALLTEMPSMINUTES:
										{
											...
											get_all_temperature_last_n_minutes(minutes,records,tempnumber,&readNodes);
